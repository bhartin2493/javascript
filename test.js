...
const stack = [];
const visited = [];
input.payBeneficiaries.map((el, i) => {
  if (el.parentBenePayId == "") {
    const arr = [el.beneficiary.businessName];
    stack.push({ orgHierarchy: arr });
    visited.push({
      id: el.beneficiaryPayId,
      arr,
    });
  }
});
while (visited.length > 0) {
  const { id, arr } = visited.pop();
  input.payBeneficiaries.map((record, j) => {
    if (record.parentBenePayId == id) {
      const newArr = [...arr, record.beneficiary.businessName];
      stack.push({ orgHierarchy: newArr });
      visited.push({
        id: record.beneficiaryPayId,
        arr: newArr,
      });
    }
  });
